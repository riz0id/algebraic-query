...
--
-- | @since 1.0.0.0
newtype SqlSelector x a = SqlSelector { sqlSelectorIx :: Int }
  deriving Show
...
--
-- | @since 1.0.0.0
field :: ∀ n x. (HasField n x) => SqlSelector x (FieldType n x)
field = SqlSelector $ case gSel (Proxy @n) (Proxy @ (Rep x)) 0 of
  Left  n -> n
